...
- Production startup currently tolerates unavailable PostgreSQL/Redis; production mode should fail closed when required security dependencies are unavailable.
...
The backend contains a provider-independent, bounded OpenAI-compatible gateway and environment-driven interviewer/evaluator model configuration. The interviewer gateway is wired to the AI question-draft workflow, while evaluation remains deterministic until the separately fine-tuned evaluator artifact is supplied. Model serving, fine-tuned artifact loading, and live Turkish/English model quality/latency tests are intentionally deferred to the model-integration phase. Fine-tuned artifacts can replace the configured model ID and version without changing the draft API contract.
