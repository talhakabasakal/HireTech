...
- `POST /api/v1/organizations/{orgId}/apps/{appId}/keys/{keyId}/rotate` - Atomically rotate key — requires `app:write`
...
| `AI_ENABLED` | `false` | Enable both Interviewer and Evaluator gateways; keep disabled until the local model endpoints are ready |
| `AI_REQUEST_TIMEOUT_SECONDS` | `30` | AI provider request timeout |
| `AI_MAX_RESPONSE_BYTES` | `1048576` | Maximum provider response size |
| `AI_INTERVIEWER_PROVIDER_LABEL` | `interviewer` | Runtime registry provider label for interviewer models |
| `AI_INTERVIEWER_BASE_URL` | `http://localhost:8001/v1` | OpenAI-compatible interviewer endpoint (Phi-4-mini-instruct) |
| `AI_INTERVIEWER_API_KEY` | *(empty)* | Optional interviewer provider key |
| `AI_INTERVIEWER_MODEL_ID` | `microsoft/Phi-4-mini-instruct` | Interviewer model identifier |
| `AI_INTERVIEWER_MODEL_VERSION` | `qlora-smoketest-v1` | Interviewer adapter/version label |
| `AI_EVALUATOR_BASE_URL` | `http://localhost:8002/v1` | OpenAI-compatible evaluator endpoint (Qwen3-4B) |
| `AI_EVALUATOR_API_KEY` | *(empty)* | Optional evaluator provider key |
| `AI_EVALUATOR_PROVIDER_LABEL` | `evaluator` | Runtime registry provider label for evaluator models |
| `AI_EVALUATOR_MODEL_ID` | `Qwen/Qwen3-4B` | Evaluator model identifier |
| `AI_EVALUATOR_MODEL_VERSION` | `qlora-smoketest-v1` | Evaluator adapter/version label |
| `MAX_BODY_BYTES` | `1048576` | Maximum request body size (1 MiB) |
...
The project includes an OpenAI-compatible local runtime for the two trained
adapters. The adapter weights are kept outside git under
`ai/runtime/adapters/{interviewer,evaluator}`; the base models are loaded from
the Hugging Face cache at runtime.
...
In a second terminal, from `hiretech_backend`, start the backend with
`AI_ENABLED=true ./dev.sh server`. The Interviewer listens on port `8001` and
the Qwen Evaluator on port `8002`. Check `http://localhost:8001/health` and
`http://localhost:8002/health` before using the GraphQL evaluation flow.

The bundled adapters are smoke-test artifacts trained on the current small
synthetic dataset. The backend validates model JSON, rubric scope, and evidence
references; if the AI endpoint is unavailable or returns an invalid contract,
it records a deterministic baseline with `-ai-fallback` and requires human
review.
