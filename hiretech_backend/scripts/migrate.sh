...
case "${1:-}" in
    up)
        ...
        for f in "$MIGRATION_DIR"/0*.sql; do
            ...
            if [[ -n "$sql" ]]; then
                if echo "$sql" | docker exec -i masterfabric-postgres psql -v ON_ERROR_STOP=1 --single-transaction -U masterfabric -d masterfabric -q; then
                    echo "  ✓ $fname"
                else
                    echo "  ✗ $fname failed; migration rolled back" >&2
                    exit 1
                fi
            fi
        done
