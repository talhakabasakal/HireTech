...
// RotateKey atomically revokes the old credential and creates a replacement.
// The raw replacement is returned once and is never persisted.
func (uc *ManageAPIKeysUseCase) RotateKey(ctx context.Context, keyID uuid.UUID) (*dto.APIKeyResponse, error) {
	current, err := uc.keyRepo.GetByID(ctx, keyID)
	if err != nil {
		return nil, err
	}
	if current == nil || !current.IsActive {
		return nil, domainErr.New(domainErr.ErrConflict, "API key is not active", nil)
	}
	rawKey, err := generateAPIKey()
	if err != nil {
		return nil, fmt.Errorf("generate API key: %w", err)
	...
	if err := uc.keyRepo.Rotate(ctx, keyID, replacement); err != nil {
		return nil, err
	}
	return &dto.APIKeyResponse{ID: replacement.ID, AppID: replacement.AppID, Name: replacement.Name, Key: rawKey, ExpiresAt: replacement.ExpiresAt, IsActive: replacement.IsActive, CreatedAt: replacement.CreatedAt}, nil
}
