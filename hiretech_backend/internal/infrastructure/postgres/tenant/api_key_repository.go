...
func (r *APIKeyRepo) Rotate(ctx context.Context, oldID uuid.UUID, replacement *model.AppAPIKey) error {
	tx, err := r.db.Begin(ctx)
	if err != nil {
		return domainErr.New(domainErr.ErrInternal, "failed to begin API key rotation", err)
	}
	defer func() { _ = tx.Rollback(ctx) }()
	var active bool
	if err := tx.QueryRow(ctx, `SELECT is_active FROM app_api_keys WHERE id=$1 FOR UPDATE`, oldID).Scan(&active); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return domainErr.New(domainErr.ErrNotFound, "api key not found", nil)
		}
		return domainErr.New(domainErr.ErrInternal, "failed to lock API key", err)
	}
	if !active {
		return domainErr.New(domainErr.ErrConflict, "API key is not active", nil)
	}
	replacement.CreatedAt = time.Now().UTC()
	if _, err := tx.Exec(ctx, `INSERT INTO app_api_keys (id, app_id, key_hash, name, scopes, expires_at, is_active, created_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`, replacement.ID, replacement.AppID, replacement.KeyHash, replacement.Name, replacement.Scopes, replacement.ExpiresAt, replacement.IsActive, replacement.CreatedAt); err != nil {
		return domainErr.New(domainErr.ErrInternal, "failed to create replacement API key", err)
	}
	if _, err := tx.Exec(ctx, `UPDATE app_api_keys SET is_active=false WHERE id=$1`, oldID); err != nil {
		return domainErr.New(domainErr.ErrInternal, "failed to revoke rotated API key", err)
	}
	if err := tx.Commit(ctx); err != nil {
		return domainErr.New(domainErr.ErrInternal, "failed to commit API key rotation", err)
	}
	return nil
}
