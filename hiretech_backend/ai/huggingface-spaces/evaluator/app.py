...
from contract_runtime import add_contract_instruction, validate_contract_text
from pydantic import BaseModel, Field
import torch
from transformers import AutoModelForCausalLM, AutoTokenizer, BitsAndBytesConfig


ROLE = os.getenv("AI_ROLE", "evaluator")
...
@spaces.GPU(duration=180)
def _gradio_complete(prompt: str) -> str:
    return _complete([{"role": "user", "content": prompt}], 512, 0)[0]


api = Server(title=f"HireTech {ROLE} AI")
...
def chat_completions(request: ChatRequest, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    messages = add_contract_instruction([{"role": item.role, "content": _content(item.content)} for item in request.messages], ROLE)
    started = time.monotonic()
    content, prompt_tokens, output_tokens = _complete(messages, request.max_tokens, request.temperature)
    try:
        validate_contract_text(content, ROLE)
    except ValueError as exc:
        raise HTTPException(status_code=502, detail=f"model output failed the {ROLE} contract") from exc
    return {
