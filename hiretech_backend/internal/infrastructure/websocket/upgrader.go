...
func originChecker(allowed []string) func(*http.Request) bool {
	if len(allowed) == 0 {
		// An absent allow-list must fail closed for browser requests. Requests
		// without an Origin header are retained for non-browser clients.
		return func(r *http.Request) bool { return r.Header.Get("Origin") == "" }
	}
