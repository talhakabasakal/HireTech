...
export class ApiAdminRepository implements AdminRepository {
  ...
  async getAuditEvents() {
    const data = await graphqlRequest<{ adminAuditEvents: { edges: Array<{ node: AuditEventDTO }> } }, { first: number }>(`query AdminAuditEvents($first: Int!) { adminAuditEvents(first: $first) { edges { node { ${auditEventFields} } } } }`, { first: 50 });
    return data.adminAuditEvents.edges.map(({ node }) => ({ ...node, result: node.result.toLowerCase() as "success" | "denied" }));
  }
