...
# Keep the project database separate from a PostgreSQL instance on the host.
# Override explicitly with HIRETECH_DB_PORT when needed.
...
# Transactional email.
# By default development uses Mailpit. Set HIRETECH_SMTP_HOST to route OTPs
# through a real provider without putting credentials in this repository.
if [[ -n "${HIRETECH_SMTP_HOST:-}" ]]; then
    export EMAIL_PROVIDER=smtp
    export SMTP_HOST="$HIRETECH_SMTP_HOST"
    export SMTP_PORT="${HIRETECH_SMTP_PORT:-587}"
    export SMTP_TLS_MODE="${HIRETECH_SMTP_TLS_MODE:-starttls}"
    export SMTP_USERNAME="${HIRETECH_SMTP_USERNAME:-}"
    export SMTP_PASSWORD="${HIRETECH_SMTP_PASSWORD:-}"
    if [[ -n "${HIRETECH_EMAIL_FROM:-}" ]]; then
        export EMAIL_FROM="$HIRETECH_EMAIL_FROM"
    elif [[ -n "${HIRETECH_SMTP_USERNAME:-}" ]]; then
        export EMAIL_FROM="HireTech <noreply@hiretech.com>"
    else
        export EMAIL_FROM="HireTech <noreply@hiretech.com>"
    fi
else
...
check_docker() {
    if ! command -v docker &>/dev/null; then
        log_error "Docker CLI is not installed. Install Docker Desktop or Docker Engine first."
        exit 1
    fi
    if ! docker info &>/dev/null; then
        if [[ -S /var/run/docker.sock ]] && DOCKER_HOST="unix:///var/run/docker.sock" docker info &>/dev/null; then
            export DOCKER_HOST="unix:///var/run/docker.sock"
            log_info "Docker Desktop context unavailable; using the local Docker Engine."
            return 0
        fi
        log_warn "Docker is not running. Start Docker Desktop/Engine and run this command again."
        if [[ "$(uname -s)" == "Darwin" ]]; then
            log_info "Attempting to start Docker Desktop on macOS..."
            open -a Docker 2>/dev/null || true
        else
            log_error "Docker must be running before the backend can start."
            exit 1
        fi
        local retries=30
...
start_infra() {
    ...
    local services=("masterfabric-postgres" "masterfabric-redis" "masterfabric-kafka" "hiretech-mailpit")
...
run_migrations() {
    ...
    for f in "$MIGRATION_DIR"/0*.sql; do
        ...
        if [[ -n "$sql" ]]; then
            if echo "$sql" | docker exec -i "$DB_CONTAINER" psql -v ON_ERROR_STOP=1 --single-transaction -U "$DB_USER" -d "$DB_NAME" -q; then
                log_ok "  $fname"
            else
                log_error "  $fname failed; migration rolled back"
                return 1
            fi
            count=$((count + 1))
        fi
...
start_server() {
    ...
    log_info "Mailpit UI available at ${BOLD}http://localhost:8025${NC}"
